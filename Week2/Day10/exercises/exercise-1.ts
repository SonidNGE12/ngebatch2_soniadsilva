...
// const queryUserpromise = (personName: string)=>{
//   new Promise((resolve)=>{
//     const result = userData.filter((user)=> user.name === personName)[0] || null
//     resolve(result)
//   })
...
// const queryFoodPromise = (foodId: number) => {
//   new Promise((resolve)=> {
//     const result = foodData.filter((food) => food.id === foodId)[ 0 ] || null
//     resolve(result)
//   })
...
const findFavouriteFoodPromise = (name:string)  => 
  new Promise((resolve) => {
    queryUserPromise(name)
    .then((person) => queryFoodPromise(person.food))
    .then((foodItem) => resolve(`Promise: ${name} likes ${foodItem.name}`))
  })
...
const queryUserPromise = (personName: string) : Promise<User> => 
  new Promise((resolve) =>{
    const result = userData.filter((user)=> user.name === personName)[0] || null
    resolve(result)
  })

 queryUserPromise('John').then(console.log) //{ name: 'John', food: 1 }

// Part 2: Refactor queryFood to use promises
// Test your refactored function by uncommenting the line below

const queryFoodPromise = (foodId : number) : Promise<Food> => 
  new Promise((resolve)=> {
    const result = foodData.filter((food) => food.id === foodId)[ 0 ] || null
    resolve(result)
  })

 queryFoodPromise(1).then(console.log) // { name: 'Pizza', id: 1 }
...
findFavouriteFoodPromise('John').then(console.log)
findFavouriteFoodPromise('Bob').then(console.log)
findFavouriteFoodPromise('Sarah').then(console.log)
findFavouriteFoodPromise('Faye').then(console.log)
