...
const getPersonalHelloMessage = () => {
    ...
    fetch('http://localhost:8080/hello/Java')
        .then(response => response.text())
        .then(data => {
        const msg = document.createElement('h3');
        msg.innerText = `The server responded: ,${data}`;
        helloDiv.appendChild(msg);
    });
};
...
helloButton.addEventListener('click', getPersonalHelloMessage);
