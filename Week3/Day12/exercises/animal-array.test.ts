...
describe('Every test has early bird at the start of the array', () => {
  ...
  it('Adds Timely Hippo and Regular Rat to an existing array that has Early Bird in it', () => {
    const array1 = addToArray(animalArray,'Timely Hippo');
    const result = addToArray(animalArray,'Regular Rat');
    expect(result).toEqual(['Early Bird','Timely Hippo','Regular Rat']);
  })
